...
type Daily = {
  ...
  pop: number; icon: string; humidityAvg: number; feelsLikeAvg: number;
};
type WeatherPayload = {
  city: string;
  current: { temp: number; feels_like: number; humidity: number; weather: string; icon: string } | null;
  daily: Daily[];
};
...
export default async function WeatherPage() {
  ...
  return (
    <main className="p-6 grid gap-6">
      <Card>
        <CardHeader><CardTitle>Weather — {data.city}</CardTitle></CardHeader>
        <CardContent className="grid gap-2 text-sm">
          {data.current ? (
            <>
              <div>Now: {data.current.temp}°C (feels {data.current.feels_like}°C) — {data.current.weather}</div>
      ...
      <Card>
        <CardHeader><CardTitle>7-Day Outlook</CardTitle></CardHeader>
        <CardContent className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-4">
          {data.daily.map(d => (
            <div key={d.date} className="rounded-2xl border p-3">
              ...
              <div className="text-xl font-semibold">{d.tempMax}° / {d.tempMin}°</div>
              ...
              <img alt="" className="h-10 w-10 mt-1" src={`https://openweathermap.org/img/wn/${d.icon}@2x.png`} />
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle>Temp & Precip (7d)</CardTitle></CardHeader>
        <CardContent>
